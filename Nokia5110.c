...
void Draw(){
	...
	y = 48/game_Type;
	  ...
	  /*for(i=0;i<game_Type;i++){
		for(j=0;j<48;j++)Nokia5110_SetPixel(x*i,j);// Turn on the specified pixel in the screen buffer.
...
unsigned long start(){
	...
	while(1){
		 ...
		 while(1)
		 {
			 if(GPIO_PORTF_RIS_R & (1 << 0)){ // SW2 // portf pin0 
				
				 break;
			 }
			 if( GPIO_PORTF_RIS_R & (1 << 4)) // SW1 // portf pin4 
			 {
	...
	if(GPIO_PORTF_RIS_R & (1 << 4)){ // SW1 pressed
	...
	if(GPIO_PORTF_RIS_R & (1 << 0)){ // SW2 pressed
